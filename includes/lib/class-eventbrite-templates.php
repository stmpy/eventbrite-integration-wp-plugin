 ...
 */
 ...
 class Eventbrite_Templates {
	...
	public function inject_page_template( $params ) {
		...
		$templates['event-list.php'] = 'Eventbrite API: Event List';
		$templates['event-details.php'] = 'Eventbrite API: Event Details';
	...
	public function check_templates( $template ) {
		...
		if ( get_the_ID() == get_page_by_path(get_option('evi_event_detail_page'))->ID ) {
			...
			// else {
				$template = plugin_dir_path( __DIR__ ) . 'templates/event-details.php';
			// }
		}

		// Check if we have a page using the Eventbrite event listing template.
		elseif ( 'event-list.php' == get_post_meta( get_the_ID(), '_wp_page_template', true ) ) {
			...
			// else {
				$template = plugin_dir_path( __DIR__ ) . 'templates/event-list.php';
			// }
		}
